...
class AdminRepairsController extends ModuleAdminController
{
    ...
    public function renderForm()
    {
        ...
        if (isset($products) && count($products) > 0) {
            foreach ($products as &$prod) {
                ...
                //get catalog price
                $prod['catalogue_price'] = Product::getPriceStatic(
                    $prod['id_product'],
                    false,
                    $prod['id_product_attribute'],
                    2,
                ...
                $prod['specific_price'] = Product::getPriceStatic(
                    $prod['id_product'],
                    false,
                    $prod['id_product_attribute'],
                    2,
        ...
        if (isset($customized_datas)) {
            foreach ($products as &$product) {
                if (!isset($customized_datas[$product['id_product']]
                    [$product['id_product_attribute']][$product['id_address_delivery']])
                ) {
                    continue;
                }
                ...
                if (version_compare(_PS_VERSION_, '1.7.0', '>=')) {
                    foreach ($customized_datas[$product['id_product']][$product['id_product_attribute']]
                             [$product['id_address_delivery']] as $customized_data) {
        ...
        $html .= $this->context->smarty
            ->fetch(_PS_MODULE_DIR_ . $this->name . '/views/templates/admin/form_repair.tpl');
    ...
    {
        $nameArray = array();
        //1 waiting for repair, 2 waiting for hardware, 3 repair in progress ,
        // 4 repaired, 5 unrepairable, 6 returned to client
    ...
    public function getYourPrice($id_cart, $id_product, $id_product_attribute, $id_customer, $get_row = false)
    {
        $sql = 'SELECT price,from_quantity FROM ' . _DB_PREFIX_ . 'specific_price WHERE id_cart=' . (int)$id_cart
            . ' AND id_product=' . (int)$id_product . ' AND id_product_attribute='
            . (int)$id_product_attribute . ' AND id_customer=' . (int)$id_customer;
    ...
    public function displayViewcustomerLink($token = null, $id = null)
    {
        if (!array_key_exists('viewcustomer', self::$cache_lang)) {
            self::$cache_lang['viewcustomer'] = $this->l('View customer');
        }
        $token = Tools::getAdminToken('AdminCustomers' . (int)Tab::getIdFromClassName('AdminCustomers')
            . (int)$this->context->cookie->id_employee);

        $new_repair = new Repair($id);
        $this->context->smarty->assign(array(
            'href' => 'index.php?controller=AdminCustomers&id_customer=' . $new_repair->id_customer
    ...
    public function displayCreateorderLink($token = null, $id = null)
    {
        if (!array_key_exists('createorder', self::$cache_lang)) {
            self::$cache_lang['createorder'] = $this->l('Create order');
        }
        $token = Tools::getAdminToken('AdminOrders' . (int)Tab::getIdFromClassName('AdminOrders')
            . (int)$this->context->cookie->id_employee);
    ...
    public function displaySendbymailLink($token = null, $id = null)
    {
        ...
        $this->context->smarty->assign(array(
            'href' => 'index.php?controller=AdminRepairs&id_repair=' . $id . '&sendbymail&token='
    ...
    public function displaySendbymailtoadminLink($token = null, $id = null)
    {
        $this->context->smarty->assign(array(
            'href' => self::$currentIndex . '&' . $this->identifier . '=' . $id
                . '&sendbymailtoadmin&token=' . ($token != null ? $token : $this->token),
    ...
    public function postProcess()
    {
        if (Tools::getIsset('ajax_customer_list')) {
            ...
            $sql = 'SELECT c.`id_customer`, c.`firstname`, c.`lastname`
                ...
                WHERE (c.firstname LIKE \'%' . pSQL($query) . '%\' OR c.lastname LIKE \'%'
                . pSQL($query) . '%\') GROUP BY c.id_customer';
        ...
        if (Tools::getIsset('ajax_product_list')) {
            ...
            $sql = 'SELECT p.`id_product`, pl.`link_rewrite`, p.`reference`, p.`price`, pl.`name`
                ...
                WHERE (pl.name LIKE \'%' . pSQL($query) . '%\' OR p.reference LIKE \'%'
                . pSQL($query) . '%\') GROUP BY p.id_product';
            ...
            foreach ($prod_list as $prod) {
                ...
                //$price = Product::getPriceStatic($prod['id_product'], false, null, 6,
                // null, false, true, 1, false, $id_customer, null, null,
                // $specific_price_output, true, true, $context, true);
        ...
        if (Tools::getIsset('ajax_address_list')) {
            ...
            $sql = 'SELECT  a.`alias`, a.`id_address`, a.`lastname`, a.`firstname`, a.`lastname`, a.`company`,
                ...
                'country_lang` cl ON (a.`id_country`=cl.`id_country` AND cl.id_lang = '
        ...
        if (Tools::getIsset('ajax_get_reduced_price')) {
            ...
            foreach ($who_is_list as $key => $value) {
                ...
                //$your_price = ($specific_price_list[$key]!='')?$specific_price_list[$key]
                //:Product::getPriceStatic($id_prod,
                // false, $id_attribute, 2, null, false, true, $qty, false, $id_customer, $id_cart, null,
                // $specific_price_output, true, true, $context, true);

                $your_price = ($specific_price_list[$key] != '') ?
                    $specific_price_list[$key] : $this->getYourPrice($id_cart, $id_prod, $id_attribute, $id_customer);
        ...
        if (Tools::getIsset('transformThisCartId')) {
            ...
            Tools::redirectAdmin(self::$currentIndex . '&id_repair=' . $new_repair->id
                . '&updaterepair&token=' . $this->token);
        }
    ...
    {
        $sql = 'SELECT c.id_cart_rule, c.code, c.description, cl.name FROM ' . _DB_PREFIX_
            . 'cart_rule c LEFT JOIN ' . _DB_PREFIX_ . 'cart_rule_lang';
        $sql .= ' cl ON (c.id_cart_rule=cl.id_cart_rule) WHERE c.active=1 AND cl.id_lang='
            . (int)$this->context->language->id . ' GROUP BY c.id_cart_rule ORDER BY c.id_cart_rule';
