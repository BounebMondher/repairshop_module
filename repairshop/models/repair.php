...
class Repair extends ObjectModel
{
    ...
    public $message;
    public $date_add;
    public $statut; //1 waiting for repair, 2 waiting for hardware,
    // 3 repair in progress , 4 repaired, 5 unrepairable, 6 returned to client
    ...
    public static function createRepair(
        ...
        foreach ($customs as $custom) {
            ...
            $sql = 'UPDATE ' . _DB_PREFIX_ . 'customization SET id_address_delivery='
                . (int)$prod_array[0]['id_address_delivery'] .
            ...
            foreach ($who_is_list as $random_id => $prod_id) {
                ...
                if (isset($add_customization_list[$random_id])) {
                    ...
                    foreach ($add_customization_list[$random_id] as $id_customization => $qtyArray) {
                        ...
                        $list_prod[$random_id]['id_customization'][$id_customization]['operator'] = ($qtyToAdd > 0)
                            ? 'up' : 'down';
                ...
                if (isset($specific_price_list[$random_id]) && $specific_price_list[$random_id] != '') {
                    ...
                    //$list_prod[$random_id]['specific_qty'] =
                    // ($specific_qty_list[$random_id] == '') ? 1 : $specific_qty_list[$random_id];
                } else {//si pas de prix specifique indique alors on enregistre
                    // le prix du produit en tant que prix specifique
                    $specific_price_output = null;
                    $price = Product::getPriceStatic(
                        $list_prod[$random_id]['id'],
                        false,
                        $list_prod[$random_id]['id_attribute'],
                        6,
                        null,
                        false,
                        true,
    ...
    public static function deleteSpecificPrice($id_cart, $id_product = null, $id_attribute = null)
    {
        ...
        if ($id_product == null) {
            ...
            $sql = 'DELETE FROM `' . _DB_PREFIX_ . 'specific_price` WHERE `id_cart` = ' . (int)$id_cart
                . ' AND id_product=' . (int)$id_product . ' AND id_attribute=' . (int)$id_attribute;
        }
        Db::getInstance()->execute($sql);
    }
    ...
    public function getDetailsTax($cart_obj)
    {
        ...
        if (isset($cart_rules) && count($cart_rules) > 0) {
            foreach ($cart_rules as $cart_rule) {
                $tax_details['discount']['total_ttc'] =
                    (!isset($tax_details['discount']['total_ttc'])
                        ? 0 : $tax_details['discount']['total_ttc']) + $cart_rule['value_real'];
                $tax_details['discount']['total_ht'] =
                    (!isset($tax_details['discount']['total_ht'])
                        ? 0 : $tax_details['discount']['total_ht']) + $cart_rule['value_tax_exc'];
            }

            $tax_details['discount']['total_tax'] =
                $tax_details['discount']['total_ttc'] - $tax_details['discount']['total_ht'];
        ...
        foreach ($products as $product) {
            ...
            $tax_details[$rate]['total_tax'] =
                (!isset($tax_details[$rate]['total_tax'])
                    ? 0 : $tax_details[$rate]['total_tax']) + $product['total_wt'] - $product['total'];
    ...
    protected function assignSummaryInformations($cart_obj)
    {
        ...
        if ($customized_datas) {
            foreach ($summary['products'] as &$product_update) {
                $product_id = (isset($product_update['id_product'])
                ...
                if (isset($customized_datas[$product_id][$product_attribute_id])) {
                    $product_update['tax_rate'] = Tax::getProductTaxRate(
                        $product_id,
                        $cart_obj->{Configuration::get('PS_TAX_ADDRESS_TYPE')}
                    );
                }
            }
        ...
        foreach ($summary['products'] as $key => &$product) {
            ...
            $product['image'] = $this->setProductImageInformations(
                $product['id_product'],
                $product['id_product_attribute']
            );
            ...
            if (is_object($product['image'])) {
                ...
                if (file_exists($img_src)) {
                    ...
                    $product['image_tag'] = '<img src="' . $img_src
                        . '" alt="" width="' . $imgSizes[0] . 'px" height="' . $imgSizes[1] . 'px"/>';
                } else {
                    $product['image_tag'] = '';
                }
            } else {
        ...
        foreach ($summary['gift_products'] as $key => &$gift_product) {
            $gift_product['image'] = $this->setProductImageInformations(
                $gift_product['id_product'],
                $gift_product['id_product_attribute']
            );
            if (is_object($gift_product['image'])) {
                $img_src = _PS_IMG_DIR_ . 'p/' . $gift_product['image']->getImgPath() . '.jpg';
                $imgSizes = $this->getFinalImgSize($img_src);
                $gift_product['image_tag'] = '<img src="' . $img_src
                    . '" alt=""  width="' . $imgSizes[0] . 'px" height="' . $imgSizes[1] . 'px" />';
            } else {
        ...
        $show_option_allow_separate_package =
            (!$cart_obj->isAllProductsInStock(true) && Configuration::get('PS_SHIP_WHEN_AVAILABLE'));
        //fix for ps 1.5.2.0 and minor
        $configSize = Configuration::get('REPAIRSHOP_IMAGESIZE');
        $smallSize = (method_exists('ImageType', 'getFormatedName'))
            ? Image::getSize(ImageType::getFormatedName('small')) : Image::getSize($configSize);
    ...
    private function getFinalImgSize($src_file)
    {
        ...
        return array($final_width, $final_height, $type);
    }
    ...
    public static function getClientRepairs($id_customer)
    {
